...
public class GoldenSpawner : MonoBehaviour
{
    [Header("Settings")]
    ...
    void Spawn()
    {
        ...
        Invoke("Spawn", UnityEngine.Random.Range(minSpawnDelay+(GameManager.Instance.CalculateScore()/15)/GameManager.Instance.CalculateGameSpeed(), maxSpawnDelay+(GameManager.Instance.CalculateScore()/10)/GameManager.Instance.CalculateGameSpeed())); 
        // Invoke 메소드를 사용하여 Spawn 메소드를 랜덤하게 호출
    }
}